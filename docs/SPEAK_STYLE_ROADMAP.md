...
- **Exit Criteria のイメージ**:
  ...
  - 主要なページ遷移は `EngrowthRouteTokens.tutorialCrossfadeDuration` 準拠で、画面内の要素切替は `EngrowthElementTokens` をベースにしつつ、オンボーディング固有の基準値（PageView 300ms、日次疑似体験3秒ステップ、チュートリアル会話600/800msの余白など）が「現行基準値」として整理されている（`MOTION_TUNING_GUIDE.md` を参照）。
